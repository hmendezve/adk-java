...
public final class BigQuerySchema {
  ...
  private static org.apache.arrow.vector.types.pojo.Field convertToArrowField(Field field) {
    ...
    ImmutableMap<String, String> metadata = null;
    if (field.getType().getStandardType() == StandardSQLTypeName.JSON) {
      metadata = ImmutableMap.of("ARROW:extension:name", "google:sqlType:json");
    }
  ...
  private static TableFieldSchema.Type convertType(StandardSQLTypeName type) {
    switch (type) {
      ...
      default -> {
        return TableFieldSchema.Type.TYPE_UNSPECIFIED;
      }
    }
  }
}
